...
//		dump($title);
	if($title){	
//	$con['title']=str_replace($title, '<font color="chocolate"> '.$title.' </font>', $con['title']);
		$con['title']=$title;
		$con['status']=1;
		$con['thumb']=['neq',''];
	}else{
...
//		dump($listNews);
		...
		$this->assign('homeReader',session('homeReader'));
        /**
         * 生成页面静态化
         */
        if($type == 'buildHtml') {
