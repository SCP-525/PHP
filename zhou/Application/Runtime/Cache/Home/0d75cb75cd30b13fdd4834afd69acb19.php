...
<?php $vo = $result['news']; ?>
  ...
  <?php if(is_array($result['advNews'])): $k = 0; $__LIST__ = $result['advNews'];if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$vo): $mod = ($k % 2 );++$k;?><div class="right-hot">
    <a target="_blank" href="<?php echo ($vo["url"]); ?>"><img src="<?php echo ($vo["thumb"]); ?>" alt="<?php echo ($vo["name"]); ?>"></a>
			...
			<div >
				<h4 style="color: burlywood;">评论</h4>
				<form method="post" id='form'>
					<label style="margin-top: 25px;"><b style="color: cadetblue;"><?php echo getLoginReadername()?></b></label>
					<textarea placeholder="不吐槽不快乐" name="content" style="width:700px;height:70px; vertical-align:top;" rows="3" maxlength="100" ></textarea>
		
					<input type="hidden" name='news_id' value="<?php echo ($id); ?>" />
					<button style="margin-top: 25px;" type="button" class="btn btn-default" id="singcms-button" onclick="detail.tc()">发表</button>
					

				</form>
				<hr />
				<div class="line" id="tc_block" style="padding:10px 0;">
                       
                    <?php if(is_array($tc)): $i = 0; $__LIST__ = $tc;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$tc): $mod = ($i % 2 );++$i;?><h4 style="color: cadetblue;"><?php echo ($tc["readername"]); ?>：</h4>
                        	<label class="" id=""style="width:700px;height:40px;outline-style: solid;outline-color: wheat;"><?php echo ($tc["tc_content"]); ?></label>
                        	<span class="time" style="margin-top: 25px;"><?php echo (date('Y-m-d g:i a',time())); ?></span><?php endforeach; endif; else: echo "" ;endif; ?>
                    
                    <!--<script>
                        $("#tc").append("<h4 style="color: cadetblue;"><?php echo ($tc["readername"]); ?>：</h4>");
                    </script>-->
				</div>
